...
// import 'package:flutter_ble_messenger/db/walet.db.dart';

enum test_covid { positive, negative }

class PassCovid extends StatefulWidget {
  const PassCovid({Key key}) : super(key: key);

  @override
  _PassCovidState createState() => _PassCovidState();
}

class _PassCovidState extends State<PassCovid> {
  ...
  Future savePC() async {
    if (_data != '' && _data != '-1') {
      if ((await codeController.findOne(_data)) == null) {
        MyQrCode qrCode = MyQrCode(_data, DateTime.now(), 'pass covid', false);
